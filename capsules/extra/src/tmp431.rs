...
use kernel::{
    ...
    hil::{i2c, sensors, time::{Alarm, AlarmClient, ConvertTicks}},
...
impl CmdBuf {
    ...
    fn read_as_temperature(&self) -> Option<i32> {
        self.inner.map(|temp| {
            ...
            let temperature = ((ms_byte << 4) + (ls_byte as i16 >> 4)) as i32; /* in range [-64 * 16 + 0; 191 * 16 + 15] = [-1024; 3071] */
...
impl<'a, S: i2c::SMBusDevice, A: Alarm<'a>> Tmp431SMBus<'a, S, A> {
    ...
    fn trigger_write_read(&self) -> Result<(), kernel::hil::i2c::Error> {
        ...
        let res = self.buffer.take().map(|buf| self.smbus_temp.smbus_write_read(buf, 1, 2)).unwrap_or(Ok(()));
        res.map_err(|(err, buf)| {self.buffer.replace(buf.try_into().unwrap()); err})
    }

    fn trigger_write(&self) -> Result<(), kernel::hil::i2c::Error> {
        // kernel::debug!("TMP431: triggering write"); 
        let res = self.buffer.take().map(|buf| self.smbus_temp.smbus_write(buf, 2)).unwrap_or(Ok(()));
        res.map_err(|(err, buf)| {self.buffer.replace(buf.try_into().unwrap()); err})
    }
    ...
    fn trigger_read(&self) -> Result<(), kernel::hil::i2c::Error> {
        ...
        let res = self.buffer.take().map(|buf| self.smbus_temp.smbus_read(buf, 1)).unwrap_or(Ok(()));
        res.map_err(|(err, buf)| {self.buffer.replace(buf.try_into().unwrap()); err})
    }
    ...
    fn command_complete(&self, buffer: &'static mut [u8], status: Result<(), i2c::Error>) {
        ...
        match self.cmd_scheduled.get() {
            ...
            Some(CmdScheduled::OneShot) => {
                ...
                self.delay.set_alarm(now, self.delay.ticks_from_ms(Self::SETTLING_DELAY_MS));
